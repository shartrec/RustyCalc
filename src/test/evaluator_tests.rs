...
mod tests {
    ...
    #[test]
    fn test_add_i() {
        let result = Evaluator::default().evaluate("2 + 2");
        assert_near!(result.unwrap(), 2.0 + 2.0);
    }

    #[test]
    fn test_add_f() {
        let result = Evaluator::default().evaluate("2.0 + 2.0");
        assert_near!(result.unwrap(), 2.0 + 2.0);
    }

    #[test]
    fn test_subtract() {
        let result = Evaluator::default().evaluate("5 - 3");
        assert_near!(result.unwrap(), 5.0 - 3.0);
    }

    #[test]
    fn test_multiply() {
        let result = Evaluator::default().evaluate("4 * 3");
        assert_near!(result.unwrap(), 4.0 * 3.0);
    }

    #[test]
    fn test_divide() {
        let result = Evaluator::default().evaluate("10 / 2");
        assert_near!(result.unwrap(), 10.0 / 2.0);
    }

    #[test]
    fn test_exponent() {
        let result = Evaluator::default().evaluate("2 ^ 3");
        assert_near!(result.unwrap(), 2.0f64.powf(3.0));
    }

    #[test]
    fn test_complex_expression() {
        let result = Evaluator::default().evaluate("3 + 5 * (2 - 8) ^ 2");
        assert_near!(result.unwrap(), 3.0 + 5.0 * (2.0 - 8.0f64).powf(2.0));
    }

    #[test]
    fn test_sine() {
        let result = Evaluator::default().evaluate("sin(0)");
        assert_near!(result.unwrap(), 0.0f64.sin());
    }

    #[test]
    fn test_cosine() {
        let result = Evaluator::default().evaluate("cos(0)");
        assert_near!(result.unwrap(), 0.0f64.cos());
    }

    #[test]
    fn test_cosine_2() {
        let result = Evaluator::default().evaluate("cos(45) * 7");
        assert_near!(result.unwrap(), 45.0f64.cos() * 7f64);
    }

    #[test]
    fn test_log() {
        let result = Evaluator::default().evaluate("10 ^ log(7)");
        assert_near!(result.unwrap(), 7.0);
    }

    #[test]
    fn test_log2() {
        let result = Evaluator::default().evaluate("2 ^ log2(7)");
        assert_near!(result.unwrap(), 7.0);
    }

    #[test]
    fn test_ln() {
        let result = Evaluator::default().evaluate("exp(ln(7))");
        assert_near!(result.unwrap(), 7.0);
    }
